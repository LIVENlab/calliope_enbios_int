...
def monopile_parameters(new_db, cutoff391,
    ...
    monopile_name = f'{park_name}_monopile'
...
def transport_offshore(new_db, cutoff391,
                       park_name: str, distance_to_shore: float, location: str,
                       offshore_type: List[Literal['monopile', 'gravity', 'tripod', 'floating']],
...
def installation_offshore(new_db, cutoff391, biosphere3,
                          park_name: str, location: str, power: float, lifetime: int,
                          offshore_type: List[Literal['monopile', 'gravity', 'tripod', 'floating']],
                          floating_platform: List[Literal['semi_sub', 'spar_buoy_concrete', 'spar_buoy_iron',
                          'spar_buoy_steel', 'tension_leg', 'barge']] = None
                          ):
...
def maintenance_offshore(new_db, cutoff391,
                         park_name: str, location: str, lifetime: int
                         ):
    """
...
def lci_offshore_turbine(new_db, cutoff391, biosphere3,
                         park_name: str, park_power: float, number_of_turbines: int, park_location: str,
                         park_coordinates: tuple,
                         manufacturer: Literal['Vestas', 'Siemens Gamesa', 'Nordex', 'Enercon', 'LM Wind'],
                         rotor_diameter: float,
    ...
    transport = transport_offshore(
        ...
        offshore_type=offshore_type,
        distance_to_shore=distance_to_shore,
        location=park_location, floating_platform=floating_platform, new_db=new_db, cutoff391=cutoff391
    )
...
def offshore_substation(new_db,
                        park_name: str, park_location: str):
    """
...
def offshore_park(new_db, cutoff391, biosphere3,
    ...
    turbine_act = lci_offshore_turbine(
        ...
        electricity_mix_steel=electricity_mix_steel,
        generator_type=generator_type, new_db=new_db, cutoff391=cutoff391, biosphere3=biosphere3)
    new_ex = park_act.new_exchange(input=turbine_act, type='technosphere', amount=number_of_turbines)
    new_ex.save()
    # add substation
    substation_act = offshore_substation(park_name=park_name, park_location=park_location, new_db=new_db)
    ...
    new_ex.save()
    # add cabling
    cabling_act = offshore_cabling(park_name=park_name, park_location=park_location, new_db=new_db)
